...
def serverErrorHandler(error):
    res = {'code': ErrorCode.SERVER_ERROR.errorCode, 'msg': ErrorCode.SERVER_ERROR.errorMsg}
    #request get,post일때 body를 가져오는 함수가 달라서 none으로 전송하고 해당 함수안에서 분기처리
    LoggingData().writeApiSuccessLog(ErrorCode.SERVER_ERROR.errorCode, None, res)
